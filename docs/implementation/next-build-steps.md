...
6. For API-backed creative guidance, deploy `backend/api/creative-interpretation.mjs` with server-only `OPENAI_API_KEY`; configure the phone with `LENSPILOT_CREATIVE_API_URL` and, if used, a non-OpenAI `LENSPILOT_CREATIVE_API_TOKEN`. Keep `LENSPILOT_ALLOW_DIRECT_OPENAI_PROVIDER=false` outside local development.
...
- The iOS app now prefers `LENSPILOT_CREATIVE_API_URL` through `LensPilotCreativeInterpretationAPIProvider`; direct OpenAI calls require the explicit local-development flag `LENSPILOT_ALLOW_DIRECT_OPENAI_PROVIDER=true`.
...
- OpenAI creative guidance is API-backed through the LensPilot backend; the same-phone camera loop still works without the API URL, and provider secrets are never accepted from or returned to the phone.
