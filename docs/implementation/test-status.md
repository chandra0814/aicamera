...
- `.env.example` validation for all server-side, phone-facing, signed-request, metrics, and iOS Creative API configuration keys, with blank secret placeholders and production-safe example limits.
- Production endpoint check validation for safe deployed `/health`, `/ready`, and optional `/metrics` probes, including unsafe-readiness rejection and metrics-token redaction.
