...
- Local Render deploy command validation for `npm run deploy:render`, including deploy-hook triggering, readiness polling, and secret-value redaction.
...
- `npm run validate` inside `shared/typescript`, covering:
  ...
  - reviewed calibration batch-import validation for validate-only mode, all-or-nothing manifest writes, duplicate sample rejection, unsafe cloud-analysis rejection, and readiness summaries
  ...
  - redacted Creative API configuration diagnostics for HTTPS backend configuration, phone bearer auth, signed requests, direct OpenAI opt-in blocking, unresolved generated config detection, and secret-free serialized status
...
- GitHub Actions Creative API container smoke, covering Docker image build, container startup, `/health`, `/ready`, protected `/metrics`, production-safety readiness, and single-phone privacy metadata.
...
- Backend `npm test` passes the deployable server wrapper smoke suite, local Render deploy validator, production env generator validator, Render deploy workflow validator, and deployment-config validator without requiring a real OpenAI key.
- Local `.\scripts\test-all.ps1` passed after pinning the Render Blueprint web service to `plan: free` and enforcing that MVP deployment guard in deployment-config validation.
...
- Local `.\scripts\test-all.ps1` passed after adding reviewed calibration batch import; Windows still skips Swift/Xcode locally.
...
- Local Docker image build smoke for `backend/Dockerfile`, because Docker Desktop/daemon is not running in this Windows workspace. GitHub Actions now runs this container smoke on Ubuntu after push.
