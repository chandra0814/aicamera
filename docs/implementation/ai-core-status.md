...
- No deployed production Creative API host, Apple App Attest verification, managed edge rate limiting, durable monitoring sink, or automated key-rotation job yet; the backend now includes production-safety preflight, signed request enforcement, replay protection, safe in-memory telemetry, key-rotation readiness metadata, and a deployed-endpoint posture check before deployment.
...
4. Deploy the Creative API runtime with `LENSPILOT_REQUIRE_PRODUCTION_SAFETY=true`, signed request enforcement, and fresh `LENSPILOT_*_ROTATED_AT` metadata, run `npm run check:production-endpoint` against the deployed route, then add Apple App Attest verification, managed edge rate limits, provider-specific quality analytics, and automated key-rotation operations.
