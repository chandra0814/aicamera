...
- No live production Creative API service URL, Apple App Attest verification, managed edge rate limiting, durable monitoring sink, or automated key-rotation job yet; the backend now includes production-safety preflight, signed request enforcement, replay protection, safe in-memory telemetry, key-rotation readiness metadata, container/Render deployment artifacts, and a deployed-endpoint posture check before deployment.
