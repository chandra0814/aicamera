...
- No live production Creative API service URL, Apple App Attest verification, managed edge rate limiting, durable monitoring sink, or automated key-rotation job yet; the backend now includes production-safety preflight, signed request enforcement, replay protection, safe in-memory telemetry, key-rotation readiness metadata, container/Render deployment artifacts, CI container smoke coverage, and a deployed-endpoint posture check before deployment.
...
4. Create the production Creative API service from `render.yaml`, enter the `sync: false` secret values and fresh `LENSPILOT_*_ROTATED_AT` metadata, run `npm run check:production-endpoint` or the GitHub production endpoint workflow against the deployed route, then add Apple App Attest verification, managed edge rate limits, provider-specific quality analytics, and automated key-rotation operations.
