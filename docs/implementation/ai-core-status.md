...
- Creative API Production Preflight: `/ready` can now enforce `LENSPILOT_REQUIRE_PRODUCTION_SAFETY=true`, and `npm run preflight:production` validates server OpenAI configuration, phone bearer authorization, signed phone requests, metrics authorization, fresh secret-rotation metadata, CORS policy, request caps, rate limits, and the single-phone privacy boundary without exposing secrets.
- Creative API Config Template Guard: backend tests now validate `.env.example` for every server/iOS Creative API key, blank secret placeholders, blank rotation metadata placeholders, production-safe default caps, signed-request settings, metrics settings, and the full iOS API route path.
- Creative API Production Endpoint Check: `npm run check:production-endpoint` verifies a deployed backend's `/health`, `/ready`, and optional `/metrics` posture without sending prompts or calling OpenAI, and fails if production safety, signed requests, metrics protection, fresh secret-rotation metadata, CORS, limits, or single-phone privacy regress.
- Creative API Deployment Artifacts: `backend/Dockerfile`, `render.yaml`, and a manual/daily GitHub production endpoint-check workflow now define a concrete production deployment path with `sync: false` secret fields and safe live-readiness verification.
- Creative API Container Smoke: the main GitHub test workflow now builds the deploy image, starts the backend container with production safety enabled, verifies `/health` and `/ready`, and confirms `/metrics` requires authorization before reporting safe telemetry.
- Creative API Render Deploy Workflow: a manual GitHub workflow now triggers a Render deploy hook and retries the safe production endpoint checker until the deployed backend reports ready.
- Creative API Local Render Deploy: `npm run deploy:render` can trigger the same deploy hook locally and poll the safe production endpoint checker without printing deploy hook, OpenAI, phone, metrics, or signing secrets.
- Creative API Production Env Generator: `npm run production-env:generate` writes a gitignored local handoff file with generated phone, signing, metrics, and rotation metadata values while leaving OpenAI, Render, and live API URL values blank.
...
- Creative API Configuration Diagnostics: the same-phone diagnostics report now shows a redacted production-readiness status for the backend URL, HTTPS, phone bearer auth, signed requests, direct OpenAI opt-in, and single-phone privacy without serializing phone tokens, signing secrets, URL query strings, metrics tokens, or OpenAI keys.
...
- No Apple App Attest verification, managed edge rate limiting, durable monitoring sink, automated key-rotation job, or funded live OpenAI provider execution yet; the backend now includes a live Render deployment path, production-safety preflight, signed request enforcement, replay protection, safe in-memory telemetry, key-rotation readiness metadata, CI container smoke coverage, a Render deploy workflow, a local Render deploy trigger, a local production-values generator, deployed-endpoint posture checks, and same-phone redacted Creative API configuration diagnostics.
...
4. Keep the Render Creative API service and GitHub secrets aligned with the generated production handoff values, then add Apple App Attest verification, managed edge rate limits, provider-specific quality analytics, durable monitoring, funded live OpenAI provider execution, and automated key-rotation operations.
