...
- No live production Creative API service URL, Apple App Attest verification, managed edge rate limiting, durable monitoring sink, or automated key-rotation job yet; the backend now includes production-safety preflight, signed request enforcement, replay protection, safe in-memory telemetry, key-rotation readiness metadata, container/Render deployment artifacts, CI container smoke coverage, a Render deploy workflow, a local Render deploy trigger, a local production-values generator, and a deployed-endpoint posture check before deployment.
...
4. Create the production Creative API service from `render.yaml`, enter the `sync: false` secret values and fresh `LENSPILOT_*_ROTATED_AT` metadata from the generated local handoff file, configure `RENDER_DEPLOY_HOOK_URL` and `LENSPILOT_CREATIVE_API_URL` as GitHub secrets, run the Render deploy workflow or `npm run check:production-endpoint` against the deployed route, then add Apple App Attest verification, managed edge rate limits, provider-specific quality analytics, and automated key-rotation operations.
